...
public class ZombieController : MonoBehaviour {
	...
	public void OnGUI()
	{
		GUI.Label (new Rect (Screen.width / 2, 11, 100, 100), "Level 1");
		GUI.Label (new Rect (Screen.width - 100, 11, 100, 100), "Score: " + congaLine.Count.ToString ());
		if (lives >= 3) {
			GUI.Box (new Rect (Screen.width / 2 + 100, 280, 30, 30), "<3");
		}
		if (lives >= 2) {
			GUI.Box (new Rect (Screen.width / 2 + 150, 280, 30, 30), "<3");
		}
		if (lives >= 1) {
			GUI.Box (new Rect (Screen.width / 2 + 200, 280, 30, 30), "<3");
		}
